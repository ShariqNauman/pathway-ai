...
-- Update the Smart Recommender functions to use a countdown system (5 -> 4 -> 3 -> 2 -> 1 -> 0)
...
BEGIN
    ...
    IF DATE(user_limits.last_reset_smart_recommender AT TIME ZONE 'UTC') < DATE(NOW() AT TIME ZONE 'UTC') THEN
        ...
        SET smart_recommender_count = daily_limit, 
            last_reset_smart_recommender = NOW()
        WHERE user_id = user_uuid;
        user_limits.smart_recommender_count := daily_limit;
    END IF;
    
    -- Return current count and whether limit is reached (0 means no uses left)
    RETURN QUERY SELECT 
...
-- Update function to decrement Smart Recommender usage
...
BEGIN
    ...
    -- If limit not reached (count > 0), decrement
