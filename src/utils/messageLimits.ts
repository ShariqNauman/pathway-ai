...
export async function checkLimitsWithoutUpdating(
  userId: string, 
  ...
  try {
    ...
    // If no limits record exists, assume zero usage
    if (!currentLimits) {
      const dailyLimit = feature === 'chat' ? 15 : feature === 'essay' ? 10 : 5;
      return {
        canUse: true,
    ...
    // Determine limits and current count based on feature
    ...
    // If should reset, user can use the feature
    if (shouldReset) {
      return {
        canUse: true,
        remaining: dailyLimit,
    ...
    // Check current usage
    const canUse = currentCount < dailyLimit;
